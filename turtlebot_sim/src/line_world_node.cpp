...
Line::Line(const Eigen::Vector2d& q1, const Eigen::Vector2d& q2) : q1(q1), q2(q2)
{
  angle = LineWorld::calculateAngle(q1, q2);
}

Line::Line(const Line& old) : q1(old.q1), q2(old.q2), angle(old.angle)
{
}
...
bool LineWorld::get_line_intersection(double p0_x, double p0_y, double p1_x, double p1_y, double p2_x, double p2_y,
...
int main(int argc, char* argv[])
{
  ...
  ROS_ASSERT(x1.size() == y1.size() && y1.size() == x2.size() && x2.size() == y2.size());
  ...
  LineWorld world(vehicle_namespace, inertial_frame, robot_frame, loop_rate, n_lines, max_distance, lines);
